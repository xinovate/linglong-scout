...
| `BriefHistory` | `src/linglong_scout/scout/brief_history.py` | 按维度跨天去重 + 重叠检测 + fallback |
| `SourcePackage` | `src/linglong_scout/scout/package.py` | 采集包定义模型 |
| `FeedbackStore` | `src/linglong_scout/scout/feedback.py` | 用户偏好存储 + 权重计算 |
| `morning_brief.md` | `src/linglong_scout/scout/prompts/morning_brief.md` | 早报 prompt 模板 |
