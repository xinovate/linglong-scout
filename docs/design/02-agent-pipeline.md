# D-02 Agent 流水线

> 状态：✅ 已实现 | 最后更新：2026-06-08 | 依赖：[D-01 数据源](01-data-sources.md)
> 属于 [D-00 设计总览](00-overview.md) 的核心流程子设计。

---

## 概述

`IngestAgent` 将采集与加工解耦：`collect.py` 负责两路并发数据采集，`agent.py` 负责格式化 + LLM 生成。MCP server 启动时自动运行 `scheduler.py` 后台任务，每天按 `collect_schedule` 时间采集原始数据。

---

## 完整流程图

```mermaid
flowchart TD
    START([IngestAgent.run]) --> COLLECT["collect.collect()<br/>两路并发采集"]

    subgraph COLLECT_MODULE["collect.py — 两路并发"]
        direction TB
        PARALLEL["asyncio.gather"]
        S1["GitHub Trending<br/>日/周/月<br/>三级 fallback"]
        S2["RSS 18 源<br/>Semaphore(3)"]
    end

    COLLECT --> DEDUP

    subgraph DEDUP["URL 去重"]
        D1["RSS seen_urls 去重"]
    end

    DEDUP --> STORE["store_raw()<br/>Redis 热 14d + JSON 文件冷"]

    STORE --> GEN["_generate()"]

    subgraph BUILD["Prompt 组装"]
        B1["加载 morning_brief.md 模板"]
        B2["注入占位符"]
        B3["{github_data}<br/>{rss_data}<br/>{company_snapshot}<br/>{history_section}<br/>..."]
    end

    GEN --> BUILD --> LLM

    subgraph LLM["LLM 调用"]
        L1["_call_llm(system_prompt, topic, date)<br/>async · httpx.AsyncClient<br/>system 顶层参数 + user 触发指令"]
        L2{成功?}
        L3["重试 (最多 2 次)"]
    end

    L1 --> L2
    L2 -->|是| SAVE
    L2 -->|否| L3
    L3 --> L1
    L3 -->|仍失败| FALLBACK["BriefHistory fallback<br/>返回历史输出"]

    SAVE["保存 BriefHistory"] --> OUTPUT(["返回 markdown 早报"])

    style S1 fill:#4CAF50,color:#fff
    style S2 fill:#2196F3,color:#fff
    style S3 fill:#FF9800,color:#fff
    style STORE fill:#00BCD4,color:#fff
    style L1 fill:#9C27B0,color:#fff
    style FALLBACK fill:#f44336,color:#fff
```

---

## MCP 调用流程（generate_brief）

```mermaid
sequenceDiagram
    participant Agent as Claude Code / OpenClaw
    participant MCP as MCP Server
    participant Redis as Redis
    participant AgentCore as IngestAgent
    participant LLM as LLM API

    Agent->>MCP: generate_brief()

    MCP->>Redis: GET scout:brief:{today}:{user_id}
    alt Brief 缓存命中
        Redis-->>MCP: markdown
        MCP-->>Agent: {cached: true, output: "..."}
    else Brief 未缓存
        MCP->>Redis: EXISTS scout:raw:{today}:*
        alt Raw 数据存在
            Redis-->>MCP: true
            MCP->>Redis: GET scout:raw:{today}:rss/github
            MCP->>AgentCore: agent.run_from_raw(package, raw)
        else Raw 数据不存在
            MCP->>AgentCore: await agent.run(package)

            par 两路并发
                AgentCore->>AgentCore: _github_trending()
            and
                AgentCore->>AgentCore: _fetch_rss_feeds()
            end

            AgentCore->>Redis: store_raw() → Redis + JSON 文件
        end

        AgentCore->>AgentCore: _generate() — Prompt 组装
        AgentCore->>LLM: await _call_llm(system_prompt, topic, date)

        alt LLM 成功
            LLM-->>AgentCore: markdown 早报
            AgentCore->>Redis: 保存 BriefHistory
        else LLM 失败
            LLM-->>AgentCore: Error
            AgentCore->>AgentCore: 重试 + BriefHistory fallback
        end

        AgentCore-->>MCP: markdown 早报
        MCP->>Redis: SET scout:brief:{today}:{user_id} (TTL 25h)
        MCP-->>Agent: {cached: false, output: "..."}
    end
```

---

## LLM 配置

| 配置 | 值 | 说明 |
|------|---|------|
| model | （通过 .scout.yml 配置） | 必填，无默认值 |
| base_url | （通过 .scout.yml 配置） | Anthropic Messages API 兼容端点 |
| protocol | auto | `auto` / `anthropic` / `openai`；URL 无法判定时显式配置 |
| max_tokens | 8000 | 输出上限 |
| timeout | 300s | 单次调用超时(2026-07-03:120→300,大数据量生成需更长) |
| retries | 2 | 失败重试次数 |
| disable_thinking | true | glm-5.1 等思考模型禁用思考,否则 max_tokens 被 reasoning 占满致 content 截断/空(2026-07-03 事故根因) |

`_call_llm()` 从 config 读 base_url（非硬编码），支持切换模型和端点。方舟 Coding Plan 的 URL 不含 `/anthropic`，必须显式设置 `llm_protocol: anthropic`。401/403 等永久认证错误不重试，避免失效 Key 连续调用。

---

## 时段标记

```python
schedule_time = config.ingest.brief_schedule_time  # "07:00"
time_range = f"{(date.today() - timedelta(days=1)).isoformat()} {schedule_time} → {today} {schedule_time}"
```

输出：`> 播报时段：2026-05-25 07:00 → 2026-05-26 07:00`

---

## 容错

| 失败点 | 处理方式 |
|--------|---------|
| 单个 RSS 源 | log warning，跳过该源 |
| GitHub Trending | 三级 fallback（OpenGithubs → HTML → Search API） |
| LLM 调用 | 重试 2 次，仍失败则 BriefHistory fallback |

---

## Prompt 大小控制

全量 RSS(399 条 × 标题+URL+摘要 200 字 ≈ 13 万字 / 6-7 万 token)会让 LLM(GLM)在大输入下输出退化——截断或空回(2026-07-03 早报事故,在"融资动态"处断、漏掉政策/开源/Top5)。`_generate` 拼装 prompt 前对 RSS 做两道收紧:

- **每源 top 15**:`_cap_per_source` 按源分组,每源只留最新 15 条(feedparser 默认最新优先),保源覆盖、砍同质冗余。399 → ~247 条。
- **摘要 120 字**:`_format_rss` 摘要 `[:200]` → `[:120]`,每条省 80 字。

合计 prompt 降到 ~6 万字(GLM 安全区)。常量在 `agent.py` 顶部:`_RSS_PER_SOURCE_LIMIT`、`_RSS_SNIPPET_LIMIT`,需要时调整。

---

## 关键文件

| 文件 | 说明 |
|------|------|
| `src/linglong/scout/agent.py` | `IngestAgent.run()` + `run_from_raw()` + `_generate()` |
| `src/linglong/scout/collect.py` | 两路并发采集：`collect()` + `SourceHealth` |
| `src/linglong/scout/scheduler.py` | 容器内自动采集调度 |
| `src/linglong/scout/raw_store.py` | 结构化原始数据存储（Redis 热 + JSON 冷） |
| `src/linglong/scout/prompts/morning_brief.md` | 早报 prompt 模板 |
| `src/linglong/config.py` | `IngestConfig` 配置模型 |
| `src/linglong/mcp/tools.py` | `generate_brief()` 缓存 + raw 路径逻辑 |
