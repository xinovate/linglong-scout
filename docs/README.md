...
graph TD
    subgraph 数据采集
        A1[OpenGithubs<br/>GitHub Trending<br/>日/周/月三段] --> D[聚合去重]
        A2[RSS 订阅源<br/>18 个源（含学术前沿 arXiv 3 源 + HF Blog）] --> D
    end
...
**Docker 部署：** 所有服务（scout + redis + rsshub）通过一个 `docker compose up -d` 启动，Docker 内部网络 `scout-net` 互联。仅 scout 暴露 `127.0.0.1:9900` 到主机，其余服务仅容器内可达。配置文件：`.scout.yml`、`.env`。
...
| [数据源架构](design/01-data-sources.md) | GitHub/RSS 两路并发 |
