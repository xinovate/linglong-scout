...
| 日期 | 主题 | 关键结论 |
|------|------|----------|
| 2026-07-03 | 早报截断事故排查 + 修复(LLM 大输入退化) | 7-3 早报在融资动态处截断;根因 GLM 在 412 items 大输入下输出退化(prompt 13 万字),非 timeout/max_tokens;立即 llm_timeout 120→300;治本 agent.py 每源 top15(_cap_per_source)+ 摘要 200→120,prompt 降到 ~6 万字,159 测试过 |
