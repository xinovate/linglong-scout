...
| 日期 | 主题 | 关键结论 |
|------|------|----------|
| 2026-07-30 | GitHub 总 Star 修复 + 评测补盲 | fallback 页面结构变化使总 Star 静默降为 0；修复解析并新增输入质量、指标忠实度检查，将采集错误与模型漏写分层归因 |
| 2026-07-29 | 生产复测 + URL 规范化 | 7 天时效修复经自然生成验证通过；来源误报已修正，3 处跨板块重复保留为 Bad Case；MCP SDK 约束在兼容的 1.x |
